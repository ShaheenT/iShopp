...
const bodySchema = z.object({
  reason: z.string().trim().min(1).max(2000).default("Verified against submitted price evidence"),
});
...
export async function POST(
  ...
  const { data, error } = await supabase.rpc("verify_community_price", {
    p_submission_id: submissionId,
    p_reason: parsedBody.data.reason,
  });

  if (error) {
    console.error("community price verification failed", error);
    const status = error.message.includes("access denied") ? 403 : error.message.includes("not found") ? 404 : 400;
    return NextResponse.json({ error: "community_price_verification_failed" }, { status });
  }
