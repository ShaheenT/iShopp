...
import { optimizeSavings, type SavingsItem, type SavingsOffer } from "@/lib/basket/savings-optimization";
import { createClient } from "@/lib/supabase/server";

const basketIdSchema = z.string().uuid();
const querySchema = z.object({
  maxStores: z.coerce.number().int().min(1).max(5).default(2),
...
export async function GET(request: Request, { params }: { params: Promise<{ basketId: string }> }) {
  ...
  if (!parsedQuery.success) return NextResponse.json({ error: "invalid_optimization_parameters" }, { status: 400 });
  ...
  if (basketError) {
    console.error("basket optimization items failed", basketError);
  ...
  if (inputError) {
    console.error("basket optimization RPC failed", inputError);
  ...
  const result = optimizeSavings(items, offers, parsedQuery.data);
  if (!result) return NextResponse.json({ error: "basket_not_fully_available" }, { status: 409 });

  return NextResponse.json({ data: result, meta: { basketId: parsedBasketId.data, ...parsedQuery.data } });
}
