...
test.describe("basket API contracts", () => {
  ...
  test("basket savings and optimization APIs use server-side intelligence", () => {
    const savings = read("app/api/basket/[basketId]/savings/route.ts");
    ...
    expect(savings).toContain("get_basket_intelligence_inputs");
    expect(optimize).toContain("get_basket_intelligence_inputs");
    expect(fulfilment).toContain("get_basket_fulfilment_inputs");
    expect(fulfilment).not.toContain("deliveryFee");
    expect(fulfilment).not.toContain("minimumOrder");
  });
});
