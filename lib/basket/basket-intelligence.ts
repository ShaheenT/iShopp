...
function summarize(
  ...
  const retailerSubtotals = [...retailerMap.values()].sort(
    (a, b) => a.retailerName.localeCompare(b.retailerName),
  );
  const total = unavailableProductIds.length
    ? null
    : Number(allocations.reduce((sum, item) => sum + item.lineTotal, 0).toFixed(2));

  return {
    strategy,
    total,
    retailerCount: retailerSubtotals.length,
    allocations,
    retailerSubtotals,
...
function bestSingleRetailer(items: BasketItem[], offers: BasketOffer[]): BasketStrategy {
  ...
  for (const retailerOffers of byRetailer.values()) {
    ...
    for (const offer of retailerOffers) {
      ...
      if (!current || offer.specialPrice < current.specialPrice || (offer.specialPrice === current.specialPrice && offer.specialId < current.specialId)) {
  ...
  return candidates[0]
    ? summarize("single_retailer", candidates[0].allocations, [])
    : summarize("unavailable", [], items.map((item) => item.productId));
}
...
function bestSplit(items: BasketItem[], offers: BasketOffer[]): BasketStrategy {
  ...
  for (const item of items) {
    const candidates = offers.filter((offer) => offer.productId === item.productId);
    candidates.sort((a, b) => a.specialPrice - b.specialPrice || a.retailerName.localeCompare(b.retailerName) || a.retailerId.localeCompare(b.retailerId));
...
function bestPractical(items: BasketItem[], offers: BasketOffer[], maxStores: number): BasketStrategy {
  ...
  function evaluate(selected: string[]) {
    ...
    for (const item of items) {
      const choices = offers
        .filter((offer) => offer.productId === item.productId && selected.includes(offer.retailerId))
        .sort((a, b) => a.specialPrice - b.specialPrice || a.retailerId.localeCompare(b.retailerId));
      if (!choices[0]) return;
  ...
  return candidates[0]
    ? summarize("practical", candidates[0].allocations, [])
    : summarize("unavailable", [], items.map((item) => item.productId));
}

export function calculateBasketIntelligence(
  items: BasketItem[],
  rawOffers: BasketOffer[],
  maxStores = 2,
): BasketIntelligence {
  ...
  return {
    ...
    singleRetailer,
    practical,
    savingsVsSingleRetailer:
      singleRetailer.total !== null && splitMinimum.total !== null
        ? Number((singleRetailer.total - splitMinimum.total).toFixed(2))
        : null,
    savingsVsSplitMinimum:
      practical.total !== null && splitMinimum.total !== null
        ? Number((practical.total - splitMinimum.total).toFixed(2))
        : null,
  };
}
