-- Anomaly detection is a review signal only. It never auto-verifies or auto-rejects a contribution.
...
begin
  ...
  if not exists (
    select 1 from public.community_price_verifiers
    where user_id = auth.uid() and is_active
  ) then
    raise exception 'community price verification access denied';
  end if;

  select user_id, product_id, retailer_id, observed_price, currency, observed_at
    into v_user_id, v_product_id, v_retailer_id, v_price, v_currency, v_observed_at
  from public.community_price_submissions
  where id = p_submission_id;

  if not found then raise exception 'community price submission not found'; end if;

  select percentile_cont(0.5) within group (order by observed_price)
    into v_median
  ...
  select count(*)::integer into v_velocity_count
  from public.community_price_submissions s
  where s.user_id = v_user_id
