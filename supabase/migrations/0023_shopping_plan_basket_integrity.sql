-- Action-layer integrity: a shopping plan must represent the user's basket exactly.
-- The API may propose an allocation, but the database remains the final authority.
-- Reject missing, extra, duplicated, or quantity-mismatched basket items before
-- any shopping-plan rows become visible to the caller.
